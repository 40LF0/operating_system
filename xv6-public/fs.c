...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT*NINDIRECT*NINDIRECT){
    ...
    bp = bread(ip->dev, addr);
    addr_t = bn /(NINDIRECT*NINDIRECT);
    a = (uint*)bp->data;
    if((addr = a[addr_t]) == 0){
      a[addr_t] = addr = balloc(ip->dev);
      log_write(bp);
    }   
    brelse(bp);
